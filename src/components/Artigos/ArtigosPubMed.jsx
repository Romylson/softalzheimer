...
const filtrosExtra = {
  todos: "",
  invivo: " AND (mice OR rats OR animal model)",
  invitro: " AND (cell culture OR in vitro)",
  ensaios: " AND clinical trial"
};

const ArtigosPubmed = ({ termo, filtro }) => {
  ...
  useEffect(() => {
    const buscarArtigos = async () => {
      ...
      try {
        const termoCompleto = `${termo}${filtrosExtra[filtro] || ""}`;
        const response = await fetch(`/api/pubmed?q=${encodeURIComponent(termoCompleto)}`);
        ...
        setErro("Erro ao buscar artigos no PubMed.");
      } finally {
        setCarregando(false);
      }
    };
      ...
      {dados.map((artigo, idx) => (
        <Card key={idx} className="mb-2">
          <Card.Body>
            ...
            <Button variant="primary" href={artigo.link} target="_blank" rel="noopener noreferrer" size="sm">
              Ver no PubMed
            </Button>
          </Card.Body>
        </Card>
